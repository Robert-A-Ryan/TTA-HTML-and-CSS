//THIS FUNCTION WILL RUN AS SOON AS THE WINDOW LOADS IN THE DOM
//IT DISABLES THE STOP BUTTON, IT ISN'T NEEDED BEFORE THE START OF A GAME.
...
//RANDOM NUMBER GENERATOR BETWEEN 2 AND 12
    ...
    diceRoll();
//PLAY DICE SOUNDS DURING ROLL
//STRING TO SHOW WHO ROLLED WHAT
    for (i=0;i<xArray.length;i++) {
        var result = i + 1;
        var pOne = xArray[0];
        var pTwo = xArray[1];
//TEMPORARY FIX TO A TIE ROLLED BY RANDOM NUMBER GENERATOR
        if (pOne == pTwo) {
            pOne = 1;
            pTwo = 2;
        }
...
//PASS THE PLAYER WHO WON THE ROLL
    return first;
}

//START THE GAME, ROLL FOR WINNER AND ACTIVE PLAYER
function startGame() {
    var xTurn = 0;
    activePlayer = rollForTurn();
//  IF DIE ROLL WAS TIE, RE-ROLL
    if(activePlayer == "") {
        activePlayer = rollForTurn();
    }
    setTimeout(function() {hideGameMsg();}, 4000);

//ASSIGN THE STATE OF CONTROL BUTTONS
//DISABLE THE START BUTTON
    var btn = document.getElementById("btnStart");
    var btn1 = document.getElementById("btnConfig");
    btnDisabled(btn);
    btnDisabled(btn1);
...
//STYLE GAME BUTTONS WHILE ENABLED
...
//USE STOP BUTTON TO END CURRENT GAME
function stopGame() {
    hideGameMsg();
    var btn = document.getElementById("btnStart");
    startEnabled(btn);
...
//RESETS LOG OF CURRENT GAME MOVES
...
//PLAYER CONFIG PANEL - PREVENT AVATAR ASSIGNMENTS FROM BEING THE SAME
...
//    var p2Index = document.getElementById("player2").selectedIndex;
  //  var p2Selected = document.getElementById("player2").options;
    //if(p1Selected[p1Index].text == p2Selected[p2Index].txt) {
//        alert("Error - Player 1 and Player 2 cannot both be assign as: "+p1Selected[p1Index].text)
  //  } else {
...
// this function is for the player uration panel and checks the  
...
//RETURNS CURRENTLY ASSIGNED AVATAR TO EACH PLAYER
function getAvatars() {
...
//RETURN ACTIVE PLAYERS AVATAR / DETERMINES THE CORRECT PIC FOR THE AVATAR
...
//GET ARRAY OF BOARD, CHECK FOR VALID SELECTION
...
//VERIFY IF SELECTED SQUARE IS EMPTY
...
//LIST PREV MOVES AND CREATE LIST
...
function check4tie() {
    ...
    if(boardState.length >= 9 && check != "That's three in a row, Player 1 wins!" && check != "That's three in a row, Player 2 wins!") {
        var txt1 = "Oh no! Nobody wins, it ends in a tie!";
        tieSound();
        writeMsg(txt1);
...
//AFTER WIN DETECTED FOLLOW THIS PROCESS TO SHOW THE WIN
...
function glowBoard(pos) {
    ...
    for (var i=0; i<squares.length;i++) {
        if (i == index0) {
            ...
            setTimeout(function() {bg1.style.backgroundColor = "rgb(244,268,66)";}, 200);
            ...
            setTimeout(function() {bg1.style.backgroundColor = "rgb(179,244,66)";}, 800);
            ...
            setTimeout(function() {bg3.style.backgroundColor = "rdb(122,244,66)";}, 400);
...
function checkWinCon5(info, squareArray) {
    ...
    for (var i in info) {
        ...
        if (info[i].charAt(0) == "4") {
            var match4Avatar = info[i].charAt(0);
        }
        if (info[i].charAt(1) == "7") {
            var match7Avatar = info[i].charAt(0);
        }
    }
...
//CLICK EVENT TO PLACE CORRECT AVATAR IN SQUARE
